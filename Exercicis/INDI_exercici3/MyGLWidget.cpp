...
void MyGLWidget::initializeGL ()
{
  ...
  posFocusPatricio = glm::vec3(4, 3.5, 4);
  posFocusArbre = glm::vec3(1, 3.5, 1);
  cas = 0;
  iniEscena ();
  iniCamera ();
}
...
void MyGLWidget::focusSCA(){
  glm::vec3 posFocus = glm::vec3(0, 0, 0);
  if (cas == 1) posFocus = posFocusPatricio;
  else if (cas == 2) posFocus = posFocusArbre;
  else if (cas == 0) posFocus = glm::vec3(View * glm::vec4(posFocus, 1.));
  glUniform3fv(posFocusLoc, 1, &posFocus[0]);
}

void MyGLWidget::canviaCas(int x){
    makeCurrent();
    cas = x;
    if (cas==0){
        glm::vec3 posFocus = glm::vec3(0, 0, 0);
